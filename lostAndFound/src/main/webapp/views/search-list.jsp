...
<body style="background: rgb(255, 255, 255); overflow-x: hidden; height: auto;">
	<!--封装容器开始-->
	<div id="pagewrapper" style="margin: 0 auto; overflow: visible; height: auto; border: 1px solid white;">
		...
		<div id="container" class="container-fluid" style="height: auto;">
			...
			<div class="listthings"
				style="width: 100%; padding: 0px; margin: -5px auto; *margin-top: -15px; height: auto; min-height: 180px; border-bottom: 1px solid #aaa;">
				<!--帖子汇总-->
				<div class="tabbable">
					<ul class="nav nav-tabs">
						<!--选项卡-->
						<li class="active" id="find-li"><a
							href="${path}/search-list.html#find" onclick="subValue(this)"
							data-toggle="tab" style="color: black;">招领信息</a></li>
						<li id="lost-li"><a href="${path}/search-list.html#lost"
							onclick="subValue(this)" data-toggle="tab" style="color: black;">寻物信息</a>
						</li>
					</ul>

					<div class="tab-content"
						style="width: 100%; min-height: 180px; height: auto; overflow: hidden; font-size: 12px;">
						<!--选项卡对应内容-->
						<div class="tab-pane active" id="find" style="width: 100%;">
							...
							<table style="border: 1px solid #aaa;"
								...
								<c:choose>
									...
									<c:otherwise>
										<tbody>
											<c:forEach items="${searchPickThingsList}" var="pickThing" varStatus="vs">
												<tr <c:if test="${vs.index mod 2 == 0 }">style="background: white;"</c:if>>
													<td style="width: 80px;">
														<script>
															document.write(parseValue("${pickThing.typeCode}"));
														</script>
													</td>
							...
							<div
								style="width: 98%; height: 50px; padding-bottom: 10px; text-align: center; margin: 0 auto;">
								<!-- url 是提供url地址的-->
								<page:createPager totalCount="${picktotalCount}"
									pageSize="${pickpageSize}" totalPage="${picktotalPage}"
									curPage="${pickpageNum}"
									url="${path}/search-list.html?keyword=${keyword}"
									valueId="page">
								</page:createPager>
							</div>
						</div>
						...
						<div class="tab-pane" id="lost" style="width: 100%;">
							...
							<table style="border: 1px solid #aaa;"
								...
								<c:choose>
									...
									<c:otherwise>
										<tbody>
											<c:forEach items="${searchLostThingsList}" var="lostThing"
												varStatus="vs">
												<tr <c:if test="${vs.index mod 2 == 0 }">style="background: white;"</c:if>>
													<td style="width: 80px;">
														<script>
															document.write(parseValue("${lostThing.typeCode}"));
														</script>
													</td>
													<td class="limit">${lostThing.name}</td>
													<td class="limit">${lostThing.lostPlace}</td>
													<td style="width: 90px;">${lostThing.publishTime}</td>
													<td>
							...
							<div id="pager" style="width: 98%; height: 50px; text-align: center; padding-bottom: 10px; margin: 0 auto;">
								<!-- url 是提供url地址的-->
								<page:createPager totalCount="${losttotalCount}"
									pageSize="${lostpageSize}" totalPage="${losttotalPage}"
									curPage="${lostpageNum}"
									url="${path}/search-list.html?keyword=${keyword}"
									valueId="page">
								</page:createPager>
