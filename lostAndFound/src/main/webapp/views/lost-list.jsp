...
<html>
<head>
	<title> ${webSiteTitle} 寻物信息页面 </title>
  ...
  <body style="background: rgb(255, 255, 255); overflow-x: hidden;height:auto;"> 
   <div id="pagewrapper" style="width:100%;margin:0 auto;overflow:visible;">
	...
	<div style="min-width:100%;max-width:1440px;margin:0 auto;">
		<!-- header start-->
		<div id="lost-list-header" style="margin-top: 0px;">
	...
	<!--------------页眉结束------------------>				  
	<!-------------主体部分开始--------------->
	<div id="container" class="container-fluid" style="max-width:100%;">
	<div style="width:100%;height:30px;">
	<span style="width:38%;float:left;padding-left:10%;font-size:14px;color:black;">
		<span>现在的位置 <a href="${path}/index.html" target="_self" style="color:#666;">主页</a>-&gt; 寻物信息页面</span>
	</span>
	<span style="width:38%;float:right;text-align:right;padding-right:10%;">
		<select name="type"	id="types" style="width:160px;">			
			<option value="${path}/lost/lost-list.html/1/${pageSize}/all">--请选择物件类型--</option>
			<c:forEach items="${thingsTypeList}" var="m">
				<option value="${path}/find/find-list.html/1/${pageSize}/${m.code}">${m.name}</option>
			</c:forEach>
		</select>
	</span>
	<script>
	 ...
	 <!--失物信息内容展示容器-->
	  ...
	  <thead>
	  <tr>
	   <th>物品类型</th> <th>物品名称</th> <th>丢失地点</th> <th>发布时间</th> <th>详情</th>
	  </tr>
	  </thead>
	  <c:choose>
	  <c:when test="${empty lostThingsList}">
	  <tbody>
	  <tr height="160" >
	  ...
	  <c:otherwise>
	  <tbody>	  
	  <c:forEach items="${lostThingsList}" var="lostThing" varStatus="vs">
		<tr
			<c:if test="${vs.index mod 2 == 0 }">style="background: white;"</c:if>>
			<td style="width:80px;"><script>document.write(parseValue("${lostThing.typeCode}"));</script></td>
			<td class="limit">${lostThing.name}</td>
			<td class="limit">${lostThing.lostPlace}</td>
			<td style="width:90px;">${lostThing.publishTime}</td>
			<%--<c:choose>
			<c:when test="${lostThing.typeCode eq 'card'}">
			<td class="limit">卡<b style="color:red;"> ${lostThing.name} </b>的卡谁捡到了</td>
			</c:when>
			<c:otherwise>
			<td class="limit"><b style="color:red;"> ${lostThing.name} </b>谁捡到了</td>
			</c:otherwise>
			</c:choose>--%>
			<td>
			<a href="${path}/lost/lost-details.html?id=${lostThing.id}"
				title="失主在哪:${lostThing.name}">查看详情
      ...
      <div style="width:100%;height:50px;text-align:center;margin:0 auto;padding-bottom:10px;">		
		...
		totalPage="${totalPage}" curPage="${pageNum}" url="${path}/lost/lost-list.html"
		valueId="types"></page:createPager>
	  </div>
    </div>
	                        <!-------------主体部分结束--------------->
