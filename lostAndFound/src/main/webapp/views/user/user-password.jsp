...
<body>
	    ...
	    <form id="user-password" method="post">
	    	<table cellpadding="2" style="margin:0 auto;">
	    		...
	    		<tr>
	    			...
	    			<td><input class="easyui-textbox" type="password" name="loginPwd" id="password"  data-options="required:true" width="120" style="width:120px;"/></td>
	    		</tr>
	    		<tr>
	    		...
	    		<tr>
	    			...
	    			<input type="hidden" name="weChat" value="${user.weChat}"/>
	    			</td>
	    		</tr>
	    	</table>
	    </form>
	...
	<script>
		...
		function submitForm(){
			var url = '${path}/user/updateLoginPwd.do';
			$('#user-password').form('submit',{  
                url: url,  
                onSubmit: function(){  
                        if($("#user-password").form("validate")) {
                        	var user = {"loginAct": "${user.loginAct}", "password":$('#password').val()};
