...
<body style="width:100%;">
	<table id="dg" title="招领贴列表" class="easyui-datagrid" width="100%"  height="100%"
             data-options="url:'${path}/user/getFindInfo.do',method:'post',toolbar: '#toolbar',border:false,fit:true,fitColumns:true,pagination:true,pagePosition:'bottom'">
         <thead>
             <tr>
                 ...
                 <th data-options="field:'pickPlace',align:'center'" width="60">捡到地点</th>
                 <th data-options="field:'pickDate',align:'center'" width="60">捡到时间</th>
   ...
   function onThingsTypeRenderer(value){
    ...
    function deleteRecord(){
    	...
    	if (rows.length > 0){
    		$.messager.confirm('消息','您确定要删除选中的'+rows.length+'条记录?',function(r){
    			if (r){
    				var ids = [];
    				var flag = true;
                    ...
                    var id = ids.join(',');
                    if(id) {
                  	  $.post("deleteFindInfo.do",{ids: id},function(result){
        					if (result.success){
