   ...
   <div id="pagewrapper" style="width:100%;margin:0 auto;overflow:visible;">
	...
	<script>
	 ...
	 <thead>
	  ...
	  <c:otherwise>
	  <tbody>	  
	  <c:forEach items="${pickThingsList}" var="pickThing" varStatus="vs">
		<tr
			<c:if test="${vs.index mod 2 == 0 }">style="background: white;"</c:if>>
			<td style="width:80px;"><script>document.write(parseValue("${pickThing.typeCode}"));</script></td>
	...
	<div style="width:100%;height:50px;text-align:center;margin:0 auto;padding-bottom:10px;">		
		<!-- url 是提供url地址的-->
		<page:createPager totalCount="${totalCount}" pageSize="${pageSize}" totalPage="${totalPage}" 
		curPage="${pageNum}" url="${path}/find/find-list.html" valueId="types"></page:createPager>
	</div>
    </div>
	<!-------------主体部分结束--------------->
