	...
	<script type="text/javascript">
		$(function () {
			...
			$("#deleteBtn").click(function () {
				...
				if ($xz.length==0){
					...
					if (confirm("确定删除所选择的数据？")){
						$.ajax({
							url:"${path}/admin/deleteFind.do?"+param,
							type:"post",
							dataType:"json",
							success:function (data) {
								...
								if(data.success){
									//删除成功后,刷新用户列表
									pageList($("#findPage").bs_pagination('getOption','currentPage'),
											$("#findPage").bs_pagination('getOption','rowsPerPage'));
								}else{
									alert("删除失败");
								}
							}
			...
			$("#editBtn").click(function () {
				...
				if($xz.length==0){
					...
					var id=$xz.val();
					$.ajax({
						url:"${path}/admin/queryFindById.do",
						type:"get",
						data:{
							"id":id
						},
						...
						success:function (data) {
							...
							$("#edit-pickDate").val(data.pickDate);
							$("#edit-pickPlace").val(data.pickPlace);
							$("#edit-state").val(data.state);
							$("#edit-storagePlace").val(data.storagePlace);
			...
			$("#updateBtn").click(function () {
				...
				$.ajax({
					url:"${path}/admin/updateFind.do",
					type:"post",
					data:{
						"id":$("#edit-id").val(),
						"state":$("#edit-state").val(),
						"name":$("#edit-name").val(),
						"pickPlace":$("#edit-pickPlace").val(),
						"pickDate":$("#edit-pickDate").val(),
						"storagePlace":$("#edit-storagePlace").val(),
					},
					dataType:"json",
					success:function (data) {
		...
		function pageList(pageNo,pageSize){
			...
			$.ajax({
				...
				success:function (data) {
					var html="";
					$.each(data.findList,function (i,find) {
						html+='<tr><td><input name="dx" value="'+find.id+'" type="checkbox"></td>';
						html+='<td>'+find.user+'</td>';
						html+='<td>'+find.name+'</td>';
						html+='<td>'+find.typeCode+'</td>';
						html+='<td>'+find.pickPlace+'</td>';
						html+='<td>'+find.pickDate+'</td>';
						html+='<td>'+find.storagePlace+'</td>';
						html+='<td>'+find.publishTime+'</td>';
						html+='<td>'+(find.state==1 ? "已认领":"未认领")+'</td></tr>';
					})
...
<!-- 编辑帖子的模态窗口 -->
<div class="modal fade" id="editFind" role="dialog">
	<div class="modal-dialog" role="document" style="width: 85%;">
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<form id="findEditForm" class="form-horizontal" role="form">
					...
					<div class="form-group">
						...
						<label for="edit-pickPlace" class="col-sm-2 control-label">捡到地点</label>
						<div class="col-sm-10" style="width: 300px;">
							<input type="text" class="form-control" id="edit-pickPlace">
						</div>

						<label for="edit-pickDate" class="col-sm-2 control-label">捡到日期</label>
						<div class="col-sm-10" style="width: 300px;">
							<input type="text" class="form-control" id="edit-pickDate" readonly>
						</div>

						<label for="edit-storagePlace" class="col-sm-2 control-label">存放地点</label>
						<div class="col-sm-10" style="width: 300px;">
							<input type="text" class="form-control" id="edit-storagePlace">
						</div>
					</div>
		    ...
		    <div class="layui-tab-content larry-personal-body clearfix mylog-info-box">
                ...
                <div class="layui-tab-item layui-field-box layui-show" >
                     ...
                     <div class="larry-table-page clearfix">
				          ...
				          <div id="findPage" class="page" style="height: 50px;"></div>
			         </div>
			    </div>
		    </div>
		</div>
