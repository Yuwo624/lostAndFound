...
function thanksList(url, pageIndex, pageSize) {
	...
	$.ajax({
		...
		success : function(data) { // data 保存提交后返回的数据，一般为 json
			if(data.total > 0) {
				for(var i = 0; i < data.thanksMessageList.length; i++) {
					// 生成0-4的随机数
					var random = parseInt(Math.random()*5);
					html = '<div class="thanks '+arr[random]+'">'
		 				+'<div class="title"><span>标题：</span><span>'+data.thanksMessageList[i].title+'</span></div>'
		 				+'<div class="substance"><span>内容：</span><span>'+data.thanksMessageList[i].content+'</span></div>'
		 				+'<div class="userName"><span>感谢人：</span><span>'+data.thanksMessageList[i].user+'</span></div>'
			 			+'<div class="leaveTime"><span>发表时间：</span><span>'+data.thanksMessageList[i].publishTime+'</span></div>'
			 			+'</div>';
				...
				if(data.total <= pageSize || total >= data.total) {
					html = '<span style="color: gray;">没有更多感谢留言了...</span>';
					$("#thanks-more").html(html);
				} else {
					globalpageIndex ++;
					html = '<a href="javascript:void(0);" class="btn btn-info"'
					+'onclick="thanksList(thanksListUrl, '+globalpageIndex+', '+globalpageSize+');">加载更多留言感谢留言...</a>';
					$("#thanks-more").html(html);
				}
			} else {
